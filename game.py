...
# czcionka uzyta w grze
font = "SaiyanSans.ttf"
font1="8-Bit-Madness.ttf"

# funkcja odpowiedzialna za wyswietlanie tekstu
...
warning = False
warning_counter = 0
warning_message = message_to_screen("!", font1, 200, red)
...
def main_menu():
    ...
    while menu:
        ...
        if cloud_x <= 800 - 1100:
            cloud_x = random.randint(600, 700)
            cloud_y = random.randint(90, 180)
        else:
            if not player.wreck_start:
                cloud_x -= 5

        game_display.blit(sprites.cloud1, (cloud1_x, cloud1_y))
        if cloud1_x <= 800 - 900:
            cloud1_x = random.randint(600, 700)
            cloud1_y = random.randint(0, 90)
        else:
            if not 1 > 3:
                cloud1_x -= 5
        if godmode:
            ...
            title = message_to_screen("Sky Fight", font, 100, red)
        controls_1 = message_to_screen("Poruszanie WASD , SPACE strzal,", font, 30, yellow)
        controls_2 = message_to_screen("SHIFT zrzut bomby,  P Pauza", font, 30, yellow)
        if selected == "Graj":
            play = message_to_screen("Graj", font, 75, white)
        else:
            play = message_to_screen("Graj", font, 75, red)
        if selected == "Wyjdz":
            game_quit = message_to_screen("Wyjdz", font, 75, white)
        else:
            game_quit = message_to_screen("Wyjdz", font, 75, red)
        ...
        game_display.blit(sprites.water, (0, 500, 800,100))
        pygame.display.update()
        pygame.display.set_caption("Sky Fight " + str(int(clock.get_fps())) + " FPS.")
        clock.tick(FPS)
...
def pause():
    ...
    paused_text = message_to_screen("PAUZA", font, 100, black)
...
def game_loop():
    ...
    game_over_selected = "Zagraj Ponownie"
    ...
    while not game_exit:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_w or event.key == pygame.K_UP:
                        ...
                        pygame.mixer.Sound.play(select)
                        game_over_selected = "Wyjdz"
                    if event.key == pygame.K_SPACE or event.key == pygame.K_RETURN:
                        pygame.mixer.Sound.play(select2)
                        if game_over_selected == "Zagraj Ponownie":
                            if score > highscore_int:
                                highscore_file = open('highscore.dat', "w")
                                highscore_file.write(str(score))
                                highscore_file.close()
                            ...
                            game_loop()
                        if game_over_selected == "Wyjdz":
                            pygame.quit()
                            quit()

            game_over_text = message_to_screen("GAME OVER", font, 100, black)
            your_score = message_to_screen("Twoj Wynik: " + str(score), font1, 50, black)
            if game_over_selected == "Zagraj Ponownie":
                play_again = message_to_screen("Zagraj Ponownie", font, 75, white)
            else:
                play_again = message_to_screen("Zagraj Ponownie", font, 75, black)
            if game_over_selected == "Wyjdz":
                game_quit = message_to_screen("Wyjdz", font, 75, white)
            else:
                game_quit = message_to_screen("Wyjdz", font, 75, black)
        ...
        # rysownaie tła i losowe rozmieszczanie chmur
        game_display.blit(sprites.background, (0, 0))
        ...
        # inicjalizacja poruszania i animacji
        player.player_init()
        enemy_heli.init()
        boat.init()
        ...
        # poruszanie wrogiego szybkiego-statku
        if spaceship_alive:
            spaceship_x -= 30
        ...
        # wykrywanie trafien we wrogi statek
        ...
        # wykrywanie trafienia w szybki-statek
        ...
        # wykrywanie trafienia przez pocisk wrogiego statku
        ...
        # wykrywanie kolizji gracza ze statkiem
        ...
        # wykrywanie kolizji z szybkim-statkiem
        ...
        # wyswietlanie wyniku
        game_display.blit(message_to_screen("Wynik: {0}".format(score), font1, 50, black), (10, 10))

        # wyswietlanie najwyzszego wyniku
        if score < highscore_int:
            hi_score_message = message_to_screen("HI-SCORE: {0}".format(highscore_int), font1, 50, black)
        else:
            highscore_file = open('highscore.dat', "w")
            highscore_file.write(str(score))
            highscore_file.close()
